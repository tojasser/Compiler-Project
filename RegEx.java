...
public class RegEx {
    ...
    //here we store NFA objects.
    private static Stack<NFA> stackNfa = new Stack<NFA> ();

    //this stack for operator characters to pop them out and perform operations on.
    ...
    public static NFA generateNFA(String regular) {
        ...
        completeNfa.getNfa().get(completeNfa.getNfa().size() - 1).setEndState(true);
    ...
    private static boolean isInputCharacter(char charAt) {
        //check by ASCII code for all the small alphabets such as (a ,b ,c ..z)
        if( (int) charAt > 96 && charAt <=122 ){
            return true;
        } else
            return false;
    }
    ...
    public static DFA generateDFA(NFA nfa) {
        ...
        while (!unprocessed.isEmpty()) {
            ...
            for (Character symbol : input) {
                ...
                moveStates (symbol, state.getDFAStates(), set1);
                removeEpsilonTransition ();
                ...
                for (int i = 0 ; i < dfa.getDfa().size(); i++) {
                    ...
                    if (st.getDFAStates().containsAll(set2)) {
                        found = true;
                        break;
                    }
                }
